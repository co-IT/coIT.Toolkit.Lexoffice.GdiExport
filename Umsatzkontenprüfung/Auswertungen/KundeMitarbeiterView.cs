...
public partial class KundeMitarbeiterView : UserControl
{
  public KundeMitarbeiterView()
  {
    ...
    treeGridView.Columns.AddRange(
      new KryptonTreeGridColumn { HeaderText = "Kunde", AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells },
  ...
  )
  {
    ...
    var gruppierteRechnungszeilen = ErweiterteRechnungszeilenBerechnen(rechnungen, kunden, mitarbeiterListe)
    ...
    foreach (var kunde in gruppierteRechnungszeilen.OrderBy(kunden => kunden.Key))
    {
      ...
      {
        var mitarbeiterNode = kundenNode.Nodes.Add("", mitarbeiter.Key.Mitarbeiter, "", "");
        var mitarbeiterSumme = 0m;

        foreach (var rechnung in mitarbeiter.OrderBy(rechnungen => rechnungen.Rechnungsnummer))
        {
          var rechnungNode = mitarbeiterNode.Nodes.Add("", "", rechnung.Rechnungsnummer, rechnung.Betrag.ToString("C"));
  ...
  )
  {
    foreach (var rechnung in rechnungen)
    {
      ...
      foreach (var zeile in rechnung.LineItems)
      {
        ...
        var mitarbeiter = MitarbeiterNameErmitteln(mitarbeiterListe, mitarbeiterNummerErgebnis.Value);
  ...
  {
    return mitarbeiter.Where(mitarbeiter => mitarbeiter.Nummer == nummer).FirstOrDefault()?.Name ?? nummer.ToString();
  }

  private string KundenNameErmitteln(List<ContactInformation> kunden, Invoice rechnung)
  {
    return kunden.SingleOrDefault(kunde => kunde.Id == rechnung.Address.ContactId)?.Company?.Name ?? string.Empty;
  }
  ...
  public void ZelleFettMarkieren(DataGridViewCell? zelle)
  {
    ...
    Maybe.From(zelle).ToResult("Zelle war leer").Tap(zelle => zelle.Style.Font = schriftartFett);
  }
}
