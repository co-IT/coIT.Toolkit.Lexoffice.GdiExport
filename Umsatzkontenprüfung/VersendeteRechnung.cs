...
public class VersendeteRechnung
{
  ...
  {
    if (!Nummer.Trim().Equals(buchung.InvoiceNumber.Trim(), StringComparison.InvariantCultureIgnoreCase))
    ...
    if (Umsatzkonto != buchung.AccountNumber)
      ergebnis.AppendLine($"Unterschiedliche Umsatzkonten: {Umsatzkonto} vs. {buchung.AccountNumber}");
