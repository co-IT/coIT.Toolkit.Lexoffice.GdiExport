...
{
  private static readonly string _rechnungenCacheDatei = "cache_rechnungen.json";
  private static readonly string _kundenCacheDatei = "cache_kunden.json";
  ...
  public async Task<List<Invoice>> RechnungenAbfragen((DateOnly Von, DateOnly Bis) zeitraum, bool cacheAktualisieren)
  {
  ...
  public static async Task<ErweiterterDateisystemCache> LadeCacheAusLokalerDatei(LexofficeService service)
  {
  ...
  {
    var fallbackCache = new TagesbasierterCache(service);
    return new ErweiterterDateisystemCache(
      fallbackCache,
      new Dictionary<DateOnly, List<Invoice>>(),
  ...
  public async Task LokalenCacheErzeugen()
  {
    ...
    await File.WriteAllTextAsync(_rechnungenCacheDatei, rechnungenCache, Encoding.UTF8, CancellationToken.None);

    var kundenCache = JsonConvert.SerializeObject(_kundenCache);
    await File.WriteAllTextAsync(_kundenCacheDatei, kundenCache, Encoding.UTF8, CancellationToken.None);
  }

  public async Task<List<Invoice>> RechnungenFürTagAbfragen(DateOnly tag, bool cacheAktualisieren)
  {
  ...
  private IEnumerable<DateOnly> TageInAbfrageZeitraumErmitteln((DateOnly Von, DateOnly Bis) zeitraum)
  {
    for (var derzeitigerTag = zeitraum.Von; derzeitigerTag <= zeitraum.Bis; derzeitigerTag = derzeitigerTag.AddDays(1))
