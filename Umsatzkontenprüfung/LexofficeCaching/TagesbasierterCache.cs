...
{
  private static readonly Dictionary<DateOnly, List<LexOfficeInvoice>> _rechnungenCache = new();
  ...
  {
    var alleVouchersInZeitraum = await _service.GetVouchersInPeriod(zeitraum.Von, zeitraum.Bis);

    var vouchersGruppiertNachTag = alleVouchersInZeitraum
      .GroupBy(voucher => voucher.VoucherDate)
      .Select(voucherGrouping => new { Tag = voucherGrouping.First().VoucherDate, Liste = voucherGrouping.ToList() })
    ...
    {
      var vouchersFürTag = vouchersGruppiertNachTag.ContainsKey(tag) ? vouchersGruppiertNachTag[tag] : [];

      var rechnungenFürTag = await RechnungenFürTagAbfragen(tag, cacheAktualisieren, vouchersFürTag);
  ...
  private async Task<List<LexOfficeInvoice>> RechnungenVonLexofficeAbrufen(List<Voucher> vouchers)
  {
