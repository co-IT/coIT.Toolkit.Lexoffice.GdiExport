...
internal partial class UmsatzkontenprüfungControl : UserControl
{
  ...
  internal UmsatzkontenprüfungControl(IMitarbeiterRepository mitarbeiterRepository, Konfiguration konfiguration)
  {
    InitializeComponent();
  ...
  private void InitiiereStandardAbfragezeitraum()
  {
    ...
    var anfangDesMonatesVor6Monaten = endeDesLetztenMonats.AddMonths(-5).GetFirstDayInMonth();
  ...
  private async Task UmsatzAbfragenKlick()
  {
    ...
    var ergebnis = await KundenUndRechnungenLaden(_konfiguration, zeitraum, cacheAktualisieren)
      ...
      .Tap(tuple => kundeMitarbeiterView.Aktualisieren(tuple.First.Rechnungen, tuple.First.Kunden, tuple.Second))
    ...
    if (ergebnis.IsFailure)
      MessageBox.Show(ergebnis.Error, "Berechnung fehlgeschlagen", MessageBoxButtons.OK, MessageBoxIcon.Error);
  ...
  private async Task<Result<List<Mitarbeiter>>> MitarbeiterLaden(Konfiguration konfiguration)
  {
    return await Result
      ...
      .Map(einstellungen => new TimeEntriesService(einstellungen.ClockodoKonfigurationErhalten()))
      ...
      .Map(
        ((List<UserWithTeam> ClockodoMitarbeiter, IReadOnlyList<Mitarbeiter> MitarbeiterListe) ergebnisse) =>
        {
          ClockodoMitarbeiterHinzufügen(ergebnisse.MitarbeiterListe.ToList(), ergebnisse.ClockodoMitarbeiter);
  ...
  private void UmsätzeAnzeigen(IEnumerable<VersendeteRechnung> umsätze)
  {
    var nachKontoGruppierteZeilen = umsätze
      ...
      .Select(gruppe => new { Umsatzkonto = gruppe.Key, Rechnungen = gruppe.OrderBy(r => r.Datum) });
    ...
    tvErgebnis.Invoke(() =>
    {
      ...
      foreach (var kontoMitRechnugen in nachKontoGruppierteZeilen)
      {
        ...
        var kontoSummeBrutto = kontoMitRechnugen.Rechnungen.Sum(rechnung => rechnung.Brutto);

        var kontoText = $"{kontoMitRechnugen.Umsatzkonto}: {kontoSummeNetto:C2} | {kontoSummeBrutto:C2}";
  ...
  {
    return kunden.Max(kunde => kunde?.Company?.Name?.Length ?? 0);
  }
  ...
  )
  {
    ...
    return rechnungen
      ...
      {
        var kundenname =
          kunden.SingleOrDefault(kunde => kunde.Id == rechnung.Address.ContactId)?.Company?.Name ?? string.Empty;
  ...
  private async void btnCsvAuswählen_Click(object sender, EventArgs e)
  {
    ...
    var ergebnis = await KundenUndRechnungenLaden(_konfiguration, ZeitraumAuslesen(), cbxCacheNeuladen.Checked)
      .Map(tuple => UmsatzlisteErstellen(tuple.Kunden, tuple.Rechnungen))
      .Map(gestellteRechnungen => AbgleichBuchhaltung.FindeAbweichungenZuRechnungen(buchungen, gestellteRechnungen))
    ...
    tvErgebnis.Invoke(() =>
    {
      ...
      foreach (var abweichungenFürErgebnis in nachFehlerGruppierteAbweichungen)
      {
        ...
        foreach (var rechnung in abweichungenFürErgebnis.Abweichungen)
          tvErgebnis.Nodes[derzeitigesErgebnis].Nodes.Add($"{rechnung.Rechnungsnr}: {rechnung.Fehler}");
