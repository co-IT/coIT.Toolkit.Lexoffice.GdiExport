...
public class AbgleichBuchhaltung
{
  ...
  {
    return buchungen.Where(buchung => !rechnungen.Any(rechnung => rechnung.Nummer == buchung.InvoiceNumber)).ToList();
  }
  ...
  )
  {
    foreach (var rechnung in rechnungen)
    {
      ...
      if (abgleichungsergebnis.IsFailure)
        yield return Abweichung.AbweichungGefunden(rechnung, abgleichungsergebnis.Error);
    }
  }
  ...
  {
    return rechnungen.Where(rechnung => !buchungen.Any(buchung => buchung.InvoiceNumber == rechnung.Nummer)).ToList();
  }
}
